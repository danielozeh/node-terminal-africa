...
test('disable carrier', async () => {
    const queryParams = `?page=1&perPage=1&active=true`
    const carriers: any = await TerminalAfrica.getCarriers(queryParams)
    const carrier: any = await TerminalAfrica.disableCarrier(carriers.data.carriers[0].carrier_id, '?domestic=true&regional=true&international=true')
    expect(carrier.status).toBe(true)
});
...
test('enable carrier', async () => {
    const queryParams = `?page=1&perPage=1&active=true`
    const carriers: any = await TerminalAfrica.getCarriers(queryParams)
    const carrier: any = await TerminalAfrica.enableCarrier(carriers.data.carriers[0].carrier_id, '?domestic=true&regional=true&international=true')
    expect(carrier.status).toBe(true)
});
